 ...
 *
 *  HORARY NUMBER WITNESS (optional, additive — not part of the owner's
 *  original 5-step rules in RKP_RULES_FROM_SARFARAZ.md): when a `horaryNumber`
 *  (1–249) is supplied, it is mapped to a sidereal longitude via an equal
 *  360°/249 division of the zodiac (NOT a reproduction of Krishnamurti's
 *  historical printed horary-number table, which this codebase does not
 *  have — this is a fresh, internally-consistent mapping built on the same
 *  real KP sub-lord machinery used everywhere else). That longitude's
 *  sub-lord is treated as one more witness: +1 if it occupies a favorable
 *  house, −1 if denial, 0 otherwise — same pattern as the 5 Classical
 *  Witnesses. Purpose: two different questions asked seconds apart, whose
 *  chart-derived signal would otherwise be identical, now get a genuinely
 *  distinguishing signal instead of silently repeating the same verdict.
 *  Fully backward compatible: omitting horaryNumber reproduces the exact
 *  original scoring and confidence formula.
 *
 * Determinism guarantee: same (chart, question, horaryNumber) → identical
 * Verdict. No Date.now(), no Math.random(), no unordered Set iteration —
 * horaryNumber, if used, MUST be generated by the caller, never in here.
 *
 * KP_CORE NOTE: the promise-layer gate, the Kotamraju filter, the
 * retrograde→DELAYED modifier, and the deterministic-id/snapshot builders
 * are genuine shared KP doctrine, not RKP-specific — they now live in
 * ./promise.ts, ./kotamraju.ts, ./retrogradeModifier.ts, and
 ...
 * @param horaryNumber  Optional 1–249 witness number (see module docstring).
 *                      Caller-generated — this function never randomizes.
...
export function judgeHorary(
  ...
  // ── STEP 4.5: Horary Number witness (optional, additive) ──────────────────
  // Not one of the 5 Classical Witnesses — see module docstring. Only applied
  // when the caller supplies a horaryNumber; omitting it reproduces the
  // original scoring exactly.
  ...
  if (horaryNumber !== undefined) {
    ...
    if ((favorable as number[]).includes(horarySubLordHouse)) {
      ...
      reasoning.push(
        step(
          4,
          `Horary №${horaryNumber}: sub-lord ${horarySubLord} occupies house ${horarySubLordHouse} ∈ favorable → (+1)`,
          1,
        ),
      );
      ...
      reasoning.push(
        step(
          4,
          `Horary №${horaryNumber}: sub-lord ${horarySubLord} occupies house ${horarySubLordHouse} ∈ denial → (−1)`,
          -1,
        ),
      );
    } else {
      reasoning.push(
        step(
          4,
          `Horary №${horaryNumber}: sub-lord ${horarySubLord} occupies house ${horarySubLordHouse} (neutral) → 0`,
          0,
        ),
      );
    }
  ...
  // ── Confidence ────────────────────────────────────────────────────────────
  // MSL Placement (2) + 6 Ruling Planet Witnesses (6) = 8, +1 more when a
  // horary number witness was applied — keeps the formula bit-identical to
  // the original when horaryNumber is omitted.
  const maxScore = horaryNumber !== undefined ? 9 : 8;
