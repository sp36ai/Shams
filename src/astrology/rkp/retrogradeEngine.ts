...
export function analyzeRetrogradChain(
  ...
  if (subLordAnalysis.isRetrograde) {
    ...
    overallConfidence = isReversalEvent ? 0.90 : 0.95;
...
function analyzeStarLordRetrograde(
  ...
  if (isNodePlanet(starLord)) {
    ...
    return {
      ...
      impact: isNodeProxyRetrograde
        ? isReversalEvent
          ? 'ENABLED_REVERSAL'
          : 'DELAYED'
        : 'NONE',
      directMotionDate: isNodeProxyRetrograde
        ? estimateDirectMotionDate(nodeStarLord!)
        : undefined,
    };
  }
